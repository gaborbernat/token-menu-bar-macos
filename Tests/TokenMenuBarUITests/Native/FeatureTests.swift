...
@Test(arguments: ControllerExit.allCases) @MainActor
private func controllerExitDoesNotRestoreFocus(exit: ControllerExit) async throws {
  var (dependencies, _) = try makeDependencies(isDemo: true)
  var restoredFocus = 0
  var visibleWhenRelaunching: Bool?
  let state = dependencies.state
  dependencies.beginPopoverActivation = { { restoredFocus += 1 } }
  dependencies.relaunch = { visibleWhenRelaunching = state.popoverVisible }
  let controller = AppController(dependencies: dependencies)
  controller.start()
  ...
  switch exit {
  case .demo: controller.setDemoMode(false)
  case .reset: await controller.settingsReset()
  case .shutdown: await controller.prepareToTerminate()
  case .stop: controller.stop()
  }

  #expect(!state.popoverVisible)
  #expect(restoredFocus == 0)
  #expect(visibleWhenRelaunching == (exit == .demo || exit == .reset ? false : nil))
}

private enum ControllerExit: CaseIterable {
  case demo, reset, shutdown, stop
}
...
@Test @MainActor func statusItemLadderStepsDownWhenHidden() async throws {
  let controller = StatusItemController(log: makeLog(), diagnosticProbeInterval: 0.01, autosaveName: nil) { _ in }
...
@Test @MainActor func onScreenFrameRequiresAVisibleWindowInsideAScreen() {
  ...
  #expect(StatusItemController.onScreenFrame(of: window, screens: screens, macOSMajor: 26) == nil)
  #expect(
  ...
  #expect(
    StatusItemController.onScreenFrame(of: window, screens: screens, occlusionVisible: { _ in true }) == nil)
...
@Test @MainActor func liveDependenciesBuildDemoGraph() async throws {
  ...
  await waitUntil { dependencies.state.sampleRevision > 0 }
...
@Test @MainActor func verificationWithoutDemoDoesNotDiscoverProvidersOrShareWidgets() async throws {
  let root = FileManager.default.temporaryDirectory.appendingPathComponent("tmb-demo-off-\(UUID().uuidString)")
  defer { try? FileManager.default.removeItem(at: root) }
  let defaults = testDefaults()
  defaults.set(false, forKey: "demoMode")
  let dependencies = try await LiveDependencies.make(
    appInfo: testAppInfo,
    paths: LiveDependencies.Paths(
      home: root, supportDirectory: root, environment: ["GITHUB_TOKEN": "verification-token"],
      userName: "verification", arguments: ["--verify-ui"], verificationProfile: VerificationProfile()),
    defaults: defaults, notificationCenter: nil, transport: NoNetworkTransport(), keychain: testKeychain,
    launchAtLogin: .inMemory())

  #expect(!dependencies.isDemo)
  #expect(dependencies.registry.ids.isEmpty)
  #expect(await dependencies.rebuildProviders(dependencies.settings).ids.isEmpty)
  #expect(dependencies.widgetStore == nil)
  #expect(dependencies.history.location == root.appendingPathComponent("usage.sqlite"))
}

@Test @MainActor func verificationRelaunchPassesItsIsolationToLaunchServices() async {
...
@Test @MainActor func relaunchLeavesTheMainQueueAvailableForShutdown() async {
  let shutdownStarted = await withCheckedContinuation { continuation in
    LiveDependencies.relaunch(
      ...
      var started = false
      Task { @MainActor in started = true }
...
func exportCommandReportsItsExitStatus(writeFails: Bool) async throws {
  let directory = FileManager.default.temporaryDirectory.appendingPathComponent("tmb-command-\(UUID().uuidString)")
  try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
  defer { try? FileManager.default.removeItem(at: directory) }
  if writeFails { try Data().write(to: directory.appendingPathComponent("menubar-light.png"), options: .atomic) }
  let destination = writeFails ? directory.appendingPathComponent("menubar-light.png") : directory
  var output: [String] = []
  var errors: [String] = []
  let invocation = ExportInvocation.files(.menuBar, directory: destination)

  let result = await ExportRunner.execute(invocation, output: { output.append($0) }, errorOutput: { errors.append($0) })

  #expect(result == (writeFails ? 1 : 0))
  if writeFails {
    #expect(output.isEmpty && errors.count == 1)
    #expect(errors[0].hasPrefix(invocation.failureMessage + ": "))
  } else {
    #expect(output == ["wrote 2 files to \(directory.path)"] && errors.isEmpty)
  }
}
...
@Test @MainActor func exportRunnerWritesOnePopoverShotPerTabAndAppearance() async throws {
  ...
  let defaults = TestDefaults()
  #expect(
    try await ExportRunner.run(
      .popover, directory: directory, settle: .zero, temporaryDirectory: directory, suite: { _ in defaults }
  ...
  for appearance in ["light", "dark"] {
    let name = "popover-history-\(appearance).png"
    let artifacts = URL(
      fileURLWithPath: try #require(ProcessInfo.processInfo.environment["TOKEN_MENU_BAR_RENDER_ARTIFACTS"]),
      isDirectory: true)
    try RenderedText(
      contains: ["Claude", "Codex", "Range values"],
      excludes: ["Loading history", "No enabled provider data"]
    ).capture(try Data(contentsOf: directory.appendingPathComponent(name)), at: artifacts.appendingPathComponent(name))
  }
  #expect(
    try FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil)
      .allSatisfy { $0.pathExtension == "png" })
  try FileManager.default.removeItem(at: directory)
}

@Test @MainActor func exportRunnerCleansIsolatedStateAfterAnImageWriteFails() async throws {
  let directory = FileManager.default.temporaryDirectory.appendingPathComponent("tmb-export-\(UUID().uuidString)")
  let blocker = directory.appendingPathComponent("popover-usage-light.png")
  ...
  #expect(
    try FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil)
      .map(\.lastPathComponent) == [blocker.lastPathComponent])
  try FileManager.default.removeItem(at: directory)
}
