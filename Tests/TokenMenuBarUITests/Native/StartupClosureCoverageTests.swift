...
struct StartupClosureCoverageTests {
  @Test @MainActor func startupClosureControllerCallbacksDriveTheirPublicBehaviors() async throws {
    ...
    await waitUntil { center.requests.contains { $0.identifier.hasSuffix(":auth") } }
    controller.start()
    ...
    #expect(center.requests.contains { $0.identifier.hasSuffix(":auth") })
  ...
  @Test @MainActor func startupClosureControllerRootCallbacksSelectAndExportHistory() async throws {
    ...
    let controller = AppController(dependencies: dependencies)
    let popover = PopoverController(content: AnyView(EmptyView()), presentsWindow: true, beginActivation: { nil })
  ...
  @Test @MainActor func startupClosureReleasedControllerActionsRemainSafe() async throws {
    ...
    let menuTarget = try #require(controller?.menuTarget)
    ...
    #expect(menuTarget.menu().items.isEmpty)
  }
  ...
  @Test @MainActor func startupClosureVerificationSnapshotFailureIsLogged() async throws {
    ...
    #expect(dependencies.log.text.contains("Could not listen for verification snapshot requests"))
  ...
  func startupClosureDeferredGraphUsesInjectedLaunchAndVerificationHooks(launchFails: Bool) async throws {
    ...
    let dependencies = try await LiveDependencies.makeDeferred(
      ...
      workspaceOpen: { _, configuration, completion in
        ...
        completion(launchFails ? .failure(CocoaError(.fileNoSuchFile)) : .success(()))
      })

    dependencies.launchAtLogin.openSettings()
    let visibleFrame = try #require(dependencies.screenVisibleFrame())
    dependencies.log.debugEnabled = true
    dependencies.settings.demoMode = false
    dependencies.relaunch()
    await waitUntil { terminator.requests == 1 || dependencies.log.text.contains("Could not relaunch") }

    #expect(opened)
    #expect(terminator.requests == (launchFails ? 0 : 1))
  ...
  @Test @MainActor func startupClosureProviderBuildReportsDeniedAndStaleGrants() async throws {
    ...
    try Data("mock OAuth source".utf8).write(to: sourceURL)
    #expect(configuration.readOAuthSource(sourceURL) == Data("mock OAuth source".utf8))
