...
@Test @MainActor func settingsRemountIsReadyWithoutADeferredFill() async throws {
  ...
  let first = host(
    AnyView(
      SettingsTab(environment: environment)
        .onPreferenceChange(SettingsContentReadyKey.self) { firstReady = $0 }),
    width: 880, height: 1_600)
  await waitUntil { firstReady }
  #expect(firstReady)
  ...
  let last = host(
    AnyView(
      SettingsTab(environment: environment)
        .onPreferenceChange(SettingsContentReadyKey.self) { lastReady = $0 }),
    width: 880, height: 3_000)
  await waitUntil { lastReady }
  #expect(lastReady)
