...
@Test(arguments: [CGRect.zero, CGRect(x: 8, y: -23, width: 22, height: 22)])
func popoverMaximumUsesTheScreenWhileItsAnchorIsUnattached(anchor: CGRect) {
  let screen = CGRect(x: 0, y: 0, width: 1440, height: 900)

  #expect(
    PopoverGeometry.maxSize(anchor: anchor, visibleFrame: screen).height == screen.height - PopoverGeometry.margin)
}

@Test func popoverMaximumKeepsADegenerateAnchorWithinASmallScreen() {
...
@Test func quotaWindowDecodesSnapshotsStoredBeforeDetailExisted() throws {
  let window = QuotaWindow(
    id: "plan", label: "Plan usage", group: .monthly, usedPercent: 40, resetsAt: fixedNow, detail: "Auto 10%")
  var document = try #require(JSONSerialization.jsonObject(with: JSONEncoder().encode(window)) as? [String: Any])
  #expect(document["detail"] as? String == "Auto 10%")
  document["detail"] = nil

  let legacy = try JSONDecoder().decode(QuotaWindow.self, from: JSONSerialization.data(withJSONObject: document))

  #expect(legacy.detail == nil)
  #expect(legacy.withDetail("Confirming reset") == window.withDetail("Confirming reset"))
  #expect(window.withDetail(nil) == legacy)
}

@Test func quotaWindowDecodesSnapshotsStoredBeforeResetPrecisionExisted() throws {
  let window = QuotaWindow(
    id: "session", label: "Session", group: .session, usedPercent: 40, resetsAt: fixedNow, resetPrecision: .day)
  var document = try #require(JSONSerialization.jsonObject(with: JSONEncoder().encode(window)) as? [String: Any])
  document["resetPrecision"] = nil
