...
@Test func nestedCoverageClaudeUsageRejectsANonobjectDocument() {
  #expect(throws: DecodingError.self) { try decodeClaudeUsage("[]") }
}
...
@Test func nestedCoverageClaudeSpendDefaultsDisabledAndReadsAutoReload() throws {
  ...
  let spend = try #require(ClaudeMapper.spend(response))

  #expect(spend.enabled == nil)
...
@Test func nestedCoverageCodexResetCreditsDefaultsEveryCount() {
  let summary = CodexAPI.ResetCreditsSummary(
    ...
    immediateResetPurchaseEligible: nil, credits: nil)
...
@Test func nestedCoverageCodexCreditEventRequiresUsage() {
  let rows: [JSONValue] = [.object(["date": .string("2026-08-29")])]
  let mapping = CodexMapper.creditEventMapping(rows)

  #expect(mapping.events.isEmpty)
  #expect(mapping.missingAmountCount == 1)
}
