...
public struct AppDependencies {
  ...
  public var openProviderLogin: (@MainActor (ProviderLoginCommand) async throws -> Void)?
  ...
  public init(
    ...
    openProviderLogin: (@MainActor (ProviderLoginCommand) async throws -> Void)? = nil
  ) {
    ...
    self.openProviderLogin = openProviderLogin
...
public final class AppController {
  private var providerLoginFlow = ProviderLoginFlow()
  ...
  private var verificationSnapshotRequests: VerificationSnapshotRequests?
  ...
  public init(dependencies: AppDependencies, initialStatusItem: NSStatusItem? = nil) {
    ...
    environment.canLaunchProviderLogin =
      dependencies.openProviderLogin != nil && !dependencies.isSandboxed && !dependencies.isDemo
  ...
  func actions() -> UIActions {
    UIActions(
      ...
      signInProvider: { [weak self] provider in Task { await self?.signInProvider(provider) } },
  ...
  public func start() {
    ...
    popover.onVisibilityChange = { [weak self] visible in
      ...
      if visible {
        self?.environment.refreshUsagePresentation()
        self?.refreshIfStale()
      }
    }
  ...
  func retryOpening(remainingAttempts: Int, previousButtonFrame: CGRect?, forcedNarrowest: Bool) -> Bool {
    if remainingAttempts == 12, !forcedNarrowest, let statusItem {
      statusItem.popoverVisible = false
      _ = statusItem.collapseToNarrowest()
      statusItem.popoverVisible = true
      statusItem.reattach()
      openPopoverAfterStatusItemAttachment(remainingAttempts: 8, forcedNarrowest: true)
      return false
    }
    guard remainingAttempts > 1 else {
      guard forcedNarrowest, dependencies.recoversOffscreenPopover else {
        popover?.close()
        return false
      }
      togglePopover()
  ...
  func rootView(_ popover: PopoverController) -> RootView {
    RootView(
      ...
      preferredContentSize: { [weak popover] in popover?.contentSize(for: $0) },
  ...
  public func stop() {
    ...
    verificationSnapshotRequests = nil
  ...
  func installObservers() {
    ...
    if let session = dependencies.verificationSession {
      ...
      if let url = dependencies.verificationSnapshotURL {
        do {
          verificationSnapshotRequests = try VerificationSnapshotRequests(snapshotURL: url) { [weak self] in
            self?.writeVerificationSnapshot()
          }
        } catch {
          dependencies.log.logWarning(
            "Could not listen for verification snapshot requests: \(error.localizedDescription)")
        }
      }
    }
  }
  ...
  private func writeVerificationSnapshot() {
    popover?.popover.contentViewController?.view.logTabHitTesting(to: dependencies.log)
    dependencies.log.flush()
  ...
  public func handleApplicationActivation() {
    let providers = providerLoginFlow.takeReturnedProviders()
    if providers.isEmpty {
      Task { [weak self] in await self?.rediscoverProviders(.applicationActivated) }
    } else {
      for provider in providers { refreshNow(provider: provider) }
    }
  }

  public func signInProvider(_ provider: ProviderID) async {
    guard !environment.isDemo, !dependencies.isSandboxed,
      let command = ProviderLoginCommand(
        provider: provider, source: dependencies.state.state(for: provider).credentialHealth.source),
      let open = dependencies.openProviderLogin
    else {
      showProviders(provider)
      return
    }
    guard providerLoginFlow.begin(provider) else { return }
    environment.providerLoginErrors[provider] = nil
    do {
      try await open(command)
      providerLoginFlow.opened(provider)
    } catch {
      providerLoginFlow.failed(provider)
      environment.providerLoginErrors[provider] =
        "Could not open sign-in. Use the login command in provider settings."
      dependencies.log.logWarning("Could not open \(provider.displayName) sign-in: \(error.localizedDescription)")
      showProviders(provider)
    }
  }
...
public enum PopoverExporter {
  ...
  public static func image(_ view: some View, dark: Bool, size: CGSize? = nil) -> NSImage? {
    image(NSHostingView(rootView: view), dark: dark, size: size)
  }

  public static func image(_ hosting: NSView, dark: Bool, size: CGSize? = nil) -> NSImage? {
  ...
  public static func png(_ view: some View, dark: Bool, size: CGSize? = nil) -> Data? {
    png(NSHostingView(rootView: view), dark: dark, size: size)
  }

  public static func png(_ hosting: NSView, dark: Bool, size: CGSize? = nil) -> Data? {
    guard let image = image(hosting, dark: dark, size: size), let rep = image.representations.first as? NSBitmapImageRep
    else {
      return nil
    }
