...
public struct SettingsTab: View {
  ...
  @State private var providerRowsReady = false
  ...
  private var context: SettingsContextPresentation {
    SettingsContextPresentation(
      settings: settings, states: environment.state.providers, isSandboxed: environment.isSandboxed,
      loginStatus: environment.launchAtLoginStatus)
  }
  ...
  public var body: some View {
    ScrollViewReader { scroll in
        ...
        VStack(alignment: .leading, spacing: 12) {
          ForEach(SettingsSection.allCases, id: \.self) { scope in
            section(scope) { sectionContent(scope, scroll: scroll) }
              .task {
                if scope == .providers {
                  providerRowsReady = true
                  focusProvider(providerFocusRequest, scroll: scroll)
                }
              }
          }
          HStack {
      ...
      .task(id: providerFocusRequest?.id) {
  ...
  func focusProvider(_ request: ProviderSettingsFocusRequest?, scroll: ScrollViewProxy) {
    ...
    withAnimation(reduceMotion ? nil : .easeOut(duration: 0.12)) {
      if let provider, providerRowsReady {
        scroll.scrollTo(SettingsFocusTarget.provider(provider), anchor: .center)
      } else {
        scroll.scrollTo(SettingsSection.providers, anchor: .center)
      }
    }
    guard providerRowsReady else { return }
  ...
  func sectionContent(_ scope: SettingsSection, scroll: ScrollViewProxy) -> AnyView {
    if scope != .about, !mountedSections.contains(scope) {
      AnyView(sectionPlaceholder(scope))
    } else {
      switch scope {
      case .about: AnyView(about)
      case .menuBar: AnyView(menuBar(scroll))
      case .providers: AnyView(providers)
      case .data: AnyView(data)
      case .notifications: AnyView(notifications)
      case .log: AnyView(LogSection(environment: environment))
      }
    }
  }
  ...
  public func setThreshold(_ threshold: Int, on: Bool) {
    ...
    settings.notifications = NotificationSettings(
      ...
      notifyOnExpiringCredits: settings.notifications.notifyOnExpiringCredits,
      notifyOnPace: settings.notifications.notifyOnPace,
      playSound: settings.notifications.playSound)
  }
  ...
  var previewModel: StatusItemModel {
    ...
    return StatusItemBuilder.build(
      StatusItemInput(
        ...
        labels: labels, now: environment.now, display: settings.usageDisplay))
  }
    ...
    VStack(alignment: .leading, spacing: 7) {
              ...
              .richHelp(
                TooltipContent(
                  ...
                  body:
                    ...
                    + "When off, use Check for Updates in the footer to request a check."
                ))
          }
        }
      }
      PanelRow("Preview") {
        Toggle("Demo data", isOn: demoModeBinding)
          .toggleStyle(.checkbox)
          .accessibilityIdentifier("settings-demo-data")
          .richHelp(
            TooltipContent(
              title: "Demo data",
              body:
                "Uses generated data in a separate history store and relaunches when changed. Verification sessions stay isolated when this is off."
            ))
      }
    }
  }
  ...
  var demoModeBinding: Binding<Bool> {
    Binding(get: { environment.isDemo }, set: { environment.actions.setDemoMode($0) })
  }
      ...
      VStack(alignment: .leading, spacing: 7) {
          ...
          Toggle("Hide account and project details", isOn: menuBarSetting(\.hidePersonalInformation))
            ...
            .richHelp(
              TooltipContent(
                ...
                body:
                  "Replaces account, workspace, and project names in the interface and exports. Stored history stays unchanged."
              ))
        }
        if settings.statusFormat == .custom {
          PanelRow("Template") {
            VStack(alignment: .leading, spacing: 3) {
              TemplateEditor(text: menuBarSetting(\.customTemplate))
                .frame(height: 54)
                .overlay {
        ...
        PanelRow("Preview") {
          StatusPreview(model: previewModel, highlightedKey: $highlightedModel) { key in
            ...
            Task { @MainActor in
              await Task.yield()
              withAnimation(reduceMotion ? nil : .easeOut(duration: 0.12)) { scroll.scrollTo(key, anchor: .center) }
            }
          }
        }
      }
  ...
  private var formatPicker: some View {
    NativeSegmentedControl(
      StatusFormat.allCases.map { (value: $0, label: $0.pickerLabel) },
    ...
    .richHelp(
      TooltipContent(
        ...
        body:
          ...
          + "Mini bars use compact gauges. Countdown adds the time to reset after the percentage. "
  ...
  private var usageDisplayPicker: some View {
    NativeSegmentedControl(
      UsageDisplay.allCases.map { (value: $0, label: $0.rawValue) },
      selection: menuBarSetting(\.usageDisplay),
  ...
  private var decimalsStepper: some View {
    Stepper("Decimals: \(settings.percentDecimals)", value: menuBarSetting(\.percentDecimals), in: 0...2)
      .accessibilityLabel("Percentage decimals")
      .accessibilityIdentifier("percent-decimals")
      .richHelp(
        TooltipContent(
  ...
  private var providers: some View {
    VStack(alignment: .leading, spacing: 7) {
      ...
      if context.showsTokenRefresh {
        ...
        Text(
          "Claude, Codex, and Gemini can save refreshed credentials. Antigravity keeps refreshed access tokens in memory. "
            + "Concurrent CLI refresh can require another sign-in."
        )
        .font(.caption)
        .semanticForeground(.secondary)
        .fixedSize(horizontal: false, vertical: true)
  ...
  var tokenRefreshProviderNames: String {
    SettingsProviderPresentation.tokenRefreshProviderNames(isSandboxed: environment.isSandboxed)
  }

  private var tokenRefreshToggle: some View {
    Toggle("Refresh expired \(tokenRefreshProviderNames) tokens on my behalf", isOn: setting(\.allowTokenRefresh))
      .toggleStyle(.checkbox)
      .richHelp(
        TooltipContent(
          title: "Token refresh",
          body:
            "Refreshes expired access tokens for \(tokenRefreshProviderNames). Claude, Codex, and Gemini may save "
            + "credentials; Antigravity keeps them in memory. If the app and a CLI rotate the same token at once, "
            + "either one can invalidate the other session and require another sign-in. When off, Token Menu Bar "
            + "never rotates tokens; an expired provider stops updating until you sign in again."
        ))
  }
  ...
  func providerRow(_ providerID: ProviderID) -> some View {
    ...
    let presentation = SettingsProviderPresentation(
      state: state, now: environment.now, hidePersonalInformation: settings.hidePersonalInformation)
    let issue = recoveryIssue(providerID)
    return VStack(alignment: .leading, spacing: 3) {
      if let error = environment.providerLoginErrors[providerID] { Banner(error) }
      ResponsivePanelLayout {
        HStack(spacing: 8) {
  ...
  @ViewBuilder private func providerRefreshStepper(_ providerID: ProviderID) -> some View {
    if settings.isProviderActive(providerID, state: environment.state.providers[providerID]) {
      Stepper(
      ...
      .accessibilityLabel("\(providerID.displayName) refresh interval in minutes")
      .accessibilityIdentifier("provider-\(providerID.rawValue)-refresh-interval")
      .richHelp(
        TooltipContent(
    ...
    VStack(alignment: .leading, spacing: 1) {
      ResponsivePanelLayout {
        HStack(alignment: .firstTextBaseline, spacing: 6) {
          ...
          Text(presentation.authentication).fixedSize(horizontal: false, vertical: true)
        }
      } narrow: {
        VStack(alignment: .leading, spacing: 1) {
          Text("Authentication").fontWeight(.medium)
      ...
      SupportingDetails("Connection details", id: "connection.\(providerID.rawValue)", state: environment.disclosures) {
        Text(credentialText(providerID)).textSelection(.enabled)
        if let location = credentialLocation(providerID) {
          Text(location).textSelection(.enabled)
        }
        Text(
          [presentation.identity, presentation.lastSuccess, presentation.service].compactMap { $0 }.joined(
            separator: " · "))
        if environment.isSandboxed {
          ForEach(resourceStates(providerID).filter { !resourceNeedsGrant($0.health) }) { access in
            providerResourceRow(access, provider: providerID)
          }
        }
      }
      if let issue {
  ...
  @ViewBuilder private func providerRecoveryButton(_ issue: ProviderRecoveryIssue?, provider: ProviderID) -> some View {
    if let issue {
      if environment.canLaunchProviderLogin, issue.kind == .credentialMissing || issue.kind == .credentialExpired,
        ProviderLoginCommand(provider: provider, source: environment.state.state(for: provider).credentialHealth.source)
          != nil
      {
        NativeActionButton(action: { environment.actions.signInProvider(provider) }) {
          Label("Sign in…", systemImage: ProviderPrimaryAction.signIn.symbol)
        }
        .richHelp(
          TooltipContent(title: "Sign in to \(provider.displayName)", body: ProviderPrimaryAction.signIn.explanation))
      }
  ...
  private var historyActions: some View {
    WrappingHStack(horizontalSpacing: 8, verticalSpacing: 6) {
      if environment.history.location != nil {
      ...
      NativeActionButton("Clear…", intent: .destructive, action: requestClearHistory)
        ...
        .confirmationDialog("Clear all stored history?", isPresented: $confirmClear) {
          ...
          Text(
            "Deletes collected samples and analytics for every account. Provider data and local transcripts are unchanged and can be imported again on a later refresh."
          )
        }
    }
    .fixedSize(horizontal: true, vertical: false)
  ...
  private var notifications: some View {
    VStack(alignment: .leading, spacing: 7) {
      ...
      PanelRow("") { Text(context.notificationSummary).font(.caption).semanticForeground(.secondary) }
      if settings.notifications.enabled {
        PanelRow("") {
          WrappingHStack(horizontalSpacing: 16, verticalSpacing: 6) {
            ...
            Toggle("Sign-in needed", isOn: setting(\.notifications.notifyOnAuthProblems))
              ...
              .richHelp(
                TooltipContent(
                  ...
                  body:
                    "Sends a notice when a provider needs authentication. Disabled while usage notifications are off."
                ))
            if context.showsExpiringCredits {
              Toggle("Expiring reset credits", isOn: setting(\.notifications.notifyOnExpiringCredits))
                .toggleStyle(.checkbox)
                .disabled(!settings.notifications.enabled)
                .richHelp(
                  TooltipContent(
                    title: "Expiring reset credits",
                    body:
                      "Sends a notice when a Codex reset credit expires within 24 hours. "
                      + "Disabled while usage notifications are off."))
            }
            Toggle("Pace", isOn: setting(\.notifications.notifyOnPace))
              .toggleStyle(.checkbox)
              .disabled(!settings.notifications.enabled)
              .richHelp(
                TooltipContent(
                  title: "Pace",
                  body:
                    "Sends a notice when a window is on course to run out before it resets, and when it pulls ahead "
                    + "of pace. Each fires once per reset period. Disabled while usage "
                    + "notifications are off."))
            Toggle("Sound", isOn: setting(\.notifications.playSound))
              .toggleStyle(.checkbox)
              .disabled(!settings.notifications.enabled)
              .richHelp(
                TooltipContent(
                  title: "Sound",
                  body: "Plays the default alert sound with each notice. Disabled while usage notifications are off."))
          }
        }
      }
    }
