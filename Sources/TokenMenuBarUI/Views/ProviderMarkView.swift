...
public struct ProviderMarkView: View {
  ...
  private let imageLoader: ProviderMarkImageLoader

  @Environment(\.colorScheme) private var colorScheme

  public init(_ provider: ProviderID, size: CGSize = defaultSize) {
    self.init(provider, size: size, imageLoader: .shared)
  }

  init(_ provider: ProviderID, size: CGSize = defaultSize, imageLoader: ProviderMarkImageLoader) {
