...
enum ClaudeAPI {
  ...
  struct UsageResponse: Decodable, Sendable, Equatable {
    ...
    let limits: [Limit]
    let droppedLimitCount: Int
    let malformedFields: [String]
    let creditResetAt: String?
    ...
    init(from decoder: any Decoder) throws {
      ...
      guard let fields = document.objectValue, fields["error"] == nil else {
        throw DecodingError.dataCorrupted(
          .init(codingPath: decoder.codingPath, debugDescription: "Expected a Claude usage object"))
      }
      ...
      let limitFields = fields["limits"]?.arrayValue ?? []
      limits = limitFields.compactMap { field in
        guard let data = try? JSONEncoder().encode(field) else { return nil }
        return try? decoder.decode(Limit.self, from: data)
      }
      droppedLimitCount = limitFields.count - limits.count
      spend = decode(Spend.self, "spend")
      extraUsage = decode(ExtraUsage.self, "extra_usage")
      creditResetAt = fields["spend"]?["resets_at"]?.stringValue ?? fields["extra_usage"]?["resets_at"]?.stringValue
      var malformed: [String] = []
      if let field = fields["limits"], !field.isNull, field.arrayValue == nil { malformed.append("limits") }
      if let field = fields["spend"], !field.isNull, spend == nil { malformed.append("spend") }
      if let field = fields["extra_usage"], !field.isNull, extraUsage == nil { malformed.append("extra_usage") }
      var windows: [String: Window] = [:]
      for key in fields.keys where !["limits", "spend", "extra_usage", "member_dashboard_available"].contains(key) {
        if let window = decode(Window.self, key) {
          windows[key] = window
        } else if let field = fields[key], !field.isNull,
          Self.knownWindowKeys[key] != nil || field["utilization"] != nil
        {
          malformed.append(key)
        }
      }
      self.windows = windows
      malformedFields = malformed.sorted()
      let recognized = Set(Self.knownWindowKeys.keys).union(["limits", "spend", "extra_usage"])
      let hasEmptySchema = fields.contains { key, value in
        recognized.contains(key) && (value.isNull || key == "limits" && value.arrayValue?.isEmpty == true)
      }
      guard
        !limits.isEmpty || !windows.isEmpty || spend != nil || extraUsage != nil
          || (hasEmptySchema && malformed.isEmpty && droppedLimitCount == 0)
      else {
        throw DecodingError.dataCorrupted(
          .init(codingPath: codingPath, debugDescription: "No readable Claude usage fields"))
      }
    }
  }
...
enum ClaudeMapper {
  ...
  static func windows(_ response: ClaudeAPI.UsageResponse) -> [QuotaWindow] {
    let structured = response.limits.map(window)
    let identities = Set(structured.map(\.id))
    let legacy = response.windows.map { key, window in
      ...
      return QuotaWindow(
        id: known?.id ?? key,
        label: known?.label
          ?? Format.humanize(key.hasPrefix("seven_day_") ? String(key.dropFirst("seven_day_".count)) : key),
        group: group,
    ...
    return structured
      + legacy.filter {
        let identity =
          $0.id.hasPrefix("seven_day_")
          ? "weekly:\(Format.slug(String($0.id.dropFirst("seven_day_".count))))" : $0.id
  ...
  static func spend(_ response: ClaudeAPI.UsageResponse) -> SpendControl? {
    ...
    let used =
      ...
      ?? extra?.usedCredits.flatMap {
        legacyMoney($0, currency: currency, exponent: exponent)
      }
    let limit =
      spend?.limit?.money(defaultCurrency: currency)
      ?? extra?.monthlyLimit.flatMap {
        legacyMoney($0, currency: currency, exponent: exponent)
      }
    let enabled = spend?.enabled ?? extra?.isEnabled
    ...
    let autoReload = spend?.autoReload?.boolValue ?? spend?.autoReload?["enabled"]?.boolValue
    return SpendControl(
      enabled: enabled,
      ...
      limit: limit,
      percent: percent,
      resetsAt: ISODate.parse(response.creditResetAt),
      limitReached: extra?.spendLimitReached ?? false,
      disabledReason: enabled == true ? nil : (spend?.disabledReason ?? extra?.disabledReason),
  ...
  private static func legacyMoney(_ amount: Double, currency: String, exponent: Int) -> Money? {
    guard let minor = Int(exactly: amount), minor >= 0, (0...9).contains(exponent) else { return nil }
  ...
  static func notices(_ response: ClaudeAPI.UsageResponse, now: Date) -> [Notice] {
    var notices: [Notice] = []
