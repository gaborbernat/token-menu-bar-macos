...
public final class Settings {
  ...
  public func modelSelection(in snapshots: [ProviderID: ProviderSnapshot]) -> [WindowKey] {
    hasCustomSelection ? selectedWindows : StatusItemBuilder.defaultSelection(snapshots)
  }
  ...
  public var usageDisplay: UsageDisplay { didSet { store(usageDisplay.rawValue, key: .usageDisplay) } }
  public var hidePersonalInformation: Bool {
    didSet { store(hidePersonalInformation, key: .hidePersonalInformation) }
  }
  public var paceWorkdays: Int {
    didSet {
      let clamped = min(max(paceWorkdays, PaceEstimate.workdayRange.lowerBound), PaceEstimate.workdayRange.upperBound)
      if clamped != paceWorkdays {
        paceWorkdays = clamped
        return
      }
      store(paceWorkdays, key: .paceWorkdays)
    }
  }

  public var presentationOptions: UsageDisplayOptions {
    UsageDisplayOptions(
      display: usageDisplay, paceWorkdays: paceWorkdays, hidePersonalInformation: hidePersonalInformation)
  }
  public var writeUsageFile: Bool { didSet { store(writeUsageFile, key: .writeUsageFile) } }
  public var firstRunCardDismissed: Bool { didSet { store(firstRunCardDismissed, key: .firstRunCardDismissed) } }
  ...
  public init(defaults: UserDefaults) {
    ...
    let knownProviders = Self.loadCodable(Set<ProviderID>.self, defaults, .knownProviders)
    let addedProviders = knownProviders.map { Set(ProviderID.allCases).subtracting($0) } ?? []
    let storedProviders = Self.loadCodable(Set<ProviderID>.self, defaults, .enabledProviders)
    enabledProviders = (storedProviders ?? Set(ProviderID.allCases)).union(addedProviders)
    ...
    let storedOrder = Self.loadCodable([ProviderID].self, defaults, .providerOrder) ?? ProviderID.allCases
    providerOrder =
      storedOrder + ProviderID.allCases.filter { addedProviders.contains($0) && !storedOrder.contains($0) }
    ...
    usageDisplay = defaults.string(forKey: Key.usageDisplay.rawValue).flatMap(UsageDisplay.init(rawValue:)) ?? .used
    hidePersonalInformation = defaults.bool(forKey: Key.hidePersonalInformation.rawValue)
    paceWorkdays =
      defaults.object(forKey: Key.paceWorkdays.rawValue) as? Int ?? PaceEstimate.workdayRange.upperBound
    writeUsageFile = defaults.bool(forKey: Key.writeUsageFile.rawValue)
    firstRunCardDismissed = defaults.bool(forKey: Key.firstRunCardDismissed.rawValue)
    loading = false
    storeCodable(Set(ProviderID.allCases), key: .knownProviders)
    if !addedProviders.isEmpty {
      storeCodable(providerOrder, key: .providerOrder)
  ...
  public var codexAnalyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence {
  ...
  public func resetToDefaults() {
    ...
    defaults.removeObject(forKey: CodexAnalyticsWatermarkPersistence.storageKey)
    ...
    usageDisplay = fresh.usageDisplay
    hidePersonalInformation = fresh.hidePersonalInformation
    paceWorkdays = fresh.paceWorkdays
    writeUsageFile = fresh.writeUsageFile
    firstRunCardDismissed = fresh.firstRunCardDismissed
  }
  ...
  enum Key: String, CaseIterable {
    ...
    case usageDisplay, hidePersonalInformation, paceWorkdays
    case writeUsageFile, firstRunCardDismissed, knownProviders
