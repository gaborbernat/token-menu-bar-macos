...
public enum Diagnostics {
  ...
  public static func report(
    ...
    if settings.hidePersonalInformation {
      let identities = ProviderID.allCases.compactMap { state.state(for: $0).snapshot?.identity }
      for (value, replacement) in identities.flatMap(\.personalReplacements) {
        report = report.replacingOccurrences(of: value, with: replacement)
      }
    }
    return LogSanitizer.redact(report)
  }
