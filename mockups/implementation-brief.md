...
History uses one metric picker, one summary, a chart with a 500-point minimum, one complete legend, selected-period CSV
export, and the existing footer. The legend contributes its ideal height to the tab's one outer scroll view; it has no
fixed-height nested scroller. All presenter state remains reachable: available models, earliest sample, selection and
hover, custom dates, paging, follow-now, and reset timestamps.
