...
test *filter:
    ...
    Scripts/with-render-checks.sh {{ if filter == "" { "16" } else { "0" } }} ${TOKEN_MENU_BAR_RENDER_CAPTURE_ONLY:+--capture-only} env -u TOKEN_MENU_BAR_TEST_DESKTOP -u TOKEN_MENU_BAR_COMPILE_NATIVE_TESTS swift test --jobs "${TOKEN_MENU_BAR_BUILD_JOBS:-2}" --no-parallel {{ if filter == "" { "" } else { "--filter " + quote(filter) } }}

# Compile native package tests without launching them
compile-native-tests:
    env -u TOKEN_MENU_BAR_TEST_DESKTOP TOKEN_MENU_BAR_COMPILE_NATIVE_TESTS=1 swift build --build-tests --jobs "${TOKEN_MENU_BAR_BUILD_JOBS:-2}" --scratch-path .build/native-compile
...
test-native:
    ...
    Scripts/check-test-isolation.sh
    test_status=0
    Scripts/with-render-checks.sh 16 ${TOKEN_MENU_BAR_RENDER_CAPTURE_ONLY:+--capture-only} swift test --jobs "${TOKEN_MENU_BAR_BUILD_JOBS:-2}" --no-parallel || test_status=$?
...
# Run functional application tests on an isolated GitHub-hosted desktop
ui: xcode
    Scripts/with-ui-artifacts.sh xcodebuild -project App/TokenMenuBar.xcodeproj -scheme TokenMenuBar-Direct -configuration Verification -destination 'platform=macOS' -derivedDataPath .build/ui-derived -clonedSourcePackagesDirPath .build/xcode-packages -only-testing:TokenMenuBarApplicationUITests test

# Run selected mock-app UI tests on this desktop
[positional-arguments]
ui-local +tests:
    bash Scripts/local-ui-tests.sh "$@"

# Run opt-in benchmarks on the latest deployed macOS
[positional-arguments]
benchmark +tests='all':
    bash Scripts/local-ui-tests.sh --suite performance "$@"
